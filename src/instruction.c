...
typedef struct {
    ...
    // Some labels are used befored it's declaration being parsed, so we don't know
    // what the label is referencing. That's why we set this flag and come back
...
Instruction *new_instruction(InstructionKind kind) {
	Instruction *instr = xcalloc(1, sizeof(Instruction));
	instr->kind = kind;
	return instr;
}
